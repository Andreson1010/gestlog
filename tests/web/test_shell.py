"""Testes do shell autenticado: navegação ativa, tema e logout."""
...
async def test_nav_nao_exibe_indicadores(
...
async def test_nav_marca_pagina_ativa(client: AsyncClient, engine: AsyncEngine) -> None:
    ...
    importar = await client.get("/importar")

    assert 'href="/" class="ativo" aria-current="page"' in home.text
    assert 'href="/importar" class="ativo" aria-current="page"' in importar.text


async def test_shell_exibe_usuario_tema_e_logout(
    client: AsyncClient, engine: AsyncEngine
) -> None:
    await _criar_usuario_com_empresa(engine, "a@empresa.com")
    await _login(client, "a@empresa.com")
    ...
    assert 'action="/auth/logout"' in resposta.text
    assert "data-tema-toggle" in resposta.text
    assert 'localStorage.getItem("gestlog-tema")' in resposta.text


async def test_home_exibe_cards_de_acao(
    client: AsyncClient, engine: AsyncEngine
) -> None:
    await _criar_usuario_com_empresa(engine, "a@empresa.com")
    await _login(client, "a@empresa.com")
    ...
    assert resposta.status_code == 200
    assert "Importar dados" in resposta.text
    assert "Chat do copiloto" in resposta.text


async def test_tokens_definem_tema_claro(client: AsyncClient) -> None:
    resposta = await client.get("/static/tokens.css")

    assert resposta.status_code == 200
    assert '[data-tema="claro"]' in resposta.text

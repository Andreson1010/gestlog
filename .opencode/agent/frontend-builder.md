...
You work in frontend folders only. What counts as frontend depends on the project stack — read AGENTS.md. It is whatever UI layer the project owns: web components/pages, a Streamlit/desktop UI, a CLI presentation layer, mobile screens, etc.
...
### Hooks and state
Keep data fetching in hooks/controllers, not in components. Use the existing pattern from the researcher's findings. They consume the API contract from the Backend Builder's summary — no hardcoded URLs, no invented shapes.
...
If tests fail: fix the implementation, not the test. If typecheck fails: fix the types. If lint fails: fix the code. If coverage drops below the project threshold: write more tests.
...
### Files Added
- `path/to/NewComponent.[ext]` — [what it renders]

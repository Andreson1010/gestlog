...
|---|---|---|---|---|
| `src/<package>/path/foo.py` | Yes | Yes | 2/2 | PERSISTED |
| `tests/path/test_foo.py` | Yes | No | — | MISSING |
| `src/<package>/path/bar.py` | Yes | Yes | 1/2 | INCOMPLETE |

### Details

**`tests/path/test_foo.py` — MISSING**
Exists but identical to `origin/main` — no change on this branch.

**`src/<package>/path/bar.py` — INCOMPLETE**
Missing marker: `def build_widget`
...
- **Report exact paths and exact missing markers.** "Some files incomplete" is not actionable. "`src/<package>/path/bar.py` is missing `def build_widget`" is.

...
spec-writer, backend-builder, frontend-builder, test-verifier, validator,
persistence-checker) live in `.opencode/agent/<name>.md` (`mode: subagent`) and are
spawned via the Task tool. Keep the agent roster and this pipeline in sync — if a
project doesn't have all of them, adapt the phases to the roles that exist.

**Model:** subagents **do not declare `model`** — they inherit the session model. If
the session runs a single model, every role uses the same one. Don't introduce
`model:` in the frontmatter unless the provider is configured in `opencode.json`.

**Project specifics:** this skill is stack-agnostic. Read `AGENTS.md` at the repo
root for the project's stack, folder layout, config module, test layout, and gate
commands. Never hardcode another project's paths or commands here.

**Code review (Phase 8):** use the **`code-reviewer`** skill over the branch diff
(`origin/main...HEAD`).
...
**What it produces:** All backend files implemented (TDD), gate checks passed (test suite ≥ coverage threshold, typecheck, lint), backend summary.
...
> If the project has no frontend, skip this phase — note it in the pipeline run.
...
2. **Open the PR** using `gh pr create`:
   ...
   cd "<worktree_path>" && gh pr create \
     ...
     --body "..."
   ```
   PR body must follow the project's PR format (see the `git-workflow` skill), typically:
...
3. **Run code review** using the **`code-reviewer`** skill on the opened PR:
...
|---|---|---|
| validator | Critical — backend logic, API shape, missing endpoint, security in backend code | backend-builder |
